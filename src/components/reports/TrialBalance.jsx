...
function Skeleton() {
  return (
    <SidebarLayout>
      ...
      <div className="tbx-wrap">
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:26, alignItems:"flex-end", flexWrap:"wrap", gap:12 }}>
          <div>
            <div className="tbx-sk" style={{ width:90, height:10, marginBottom:10 }}/>
            <div className="tbx-sk" style={{ width:200, height:28 }}/>
          </div>
          <div className="tbx-sk" style={{ width:130, height:38, borderRadius:10 }}/>
        </div>
        <div className="tbx-cards" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:20 }}>
          {[0,1,2].map(i=>(
            <div key={i} style={{ background:"#fff", border:"1.5px solid #e2e8f0", borderRadius:14, padding:"18px 20px" }}>
              <div className="tbx-sk" style={{ width:"50%", height:10, marginBottom:11 }}/>
              <div className="tbx-sk" style={{ width:"70%", height:22, marginBottom:10 }}/>
              <div className="tbx-sk" style={{ width:"60%", height:9 }}/>
            </div>
          ))}
        </div>
        <div className="tbx-report-panel">
          <div style={{ padding:"20px 24px", background:"#f8fafc", borderBottom:"1.5px solid #f1f5f9", display:"flex", justifyContent:"center", gap:14, alignItems:"center" }}>
            <div className="tbx-sk" style={{ width:44, height:44, borderRadius:8 }}/>
            <div>
              <div className="tbx-sk" style={{ width:150, height:14, marginBottom:7 }}/>
              <div className="tbx-sk" style={{ width:110, height:11 }}/>
            </div>
          </div>
          <div style={{ padding:"0" }}>
            ...
            {[0,1,2,3,4,5,6].map(r=>(
              <div key={r} style={{ display:"flex", justifyContent:"space-between", padding:"10px 18px", borderBottom:"1px solid #f8fafc" }}>
                ...
                <div style={{ display:"flex", gap:40 }}>
                  <div className="tbx-sk" style={{ width:80, height:13 }}/>
                  <div className="tbx-sk" style={{ width:80, height:13 }}/>
                </div>
              </div>
            ))}
  ...
  return (
    <SidebarLayout>
      ...
      <div className="tbx-wrap">
        ...
        {/* ── Summary cards ── */}
        <div className="tbx-cards" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:18 }}>

          {/* Total Debit */}
          <div className="tbx-card" style={{ "--tbx-accent":"#4f46e5" }}>
          ...
          {/* Total Credit */}
          <div className="tbx-card" style={{ "--tbx-accent":"#059669" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center" }}>
            <div className={`tbx-match-chip tbx-match-chip-${isBalanced ? "ok" : "warn"}`}>
              {isBalanced ? (
                <><svg width={15} height={15} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  Debits = Credits · Books are balanced</>
              ) : (
                <><svg width={15} height={15} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
        ...
        <div ref={reportRef} className="tbx-report-panel">

          {/* company header */}
          <div className="tbx-report-head">
            <img
            ...
            <div style={{ textAlign:"center" }}>
              <p className="tbx-company-name">Al Rehman Rice Mills</p>
              <p className="tbx-report-date">
                Trial Balance &middot; As at{" "}
          ...
          <table className="tbx-table">
            <thead>
              <tr>
                <th style={{ textAlign:"left", paddingLeft:18 }}>Account</th>
                <th className="tbx-th-num">Debit</th>
                <th className="tbx-th-num">Credit</th>
              </tr>
            </thead>
            <tbody>
              {data.groups?.map((group) => {
                const groupDebit  = group.rows?.reduce((s, r) => s + (Number(r.debit)  || 0), 0) || 0;
                const groupCredit = group.rows?.reduce((s, r) => s + (Number(r.credit) || 0), 0) || 0;
                return (
                  <React.Fragment key={group.type}>
                    {/* group header */}
                    <tr className="tbx-group-row">
                      <td colSpan={3}>{group.type}</td>
                    </tr>

                    {/* account rows */}
                    {group.rows?.map((r) => (
                      <tr key={r.id} className="tbx-row">
                        <td style={{ paddingLeft:28 }}>{r.name}</td>
                        <td className={r.debit ? "tbx-td-num" : "tbx-td-num tbx-td-num-empty"}>
                          {r.debit ? fmt(r.debit) : "—"}
                        </td>
                        <td className={r.credit ? "tbx-td-num" : "tbx-td-num tbx-td-num-empty"}>
                    ...
                    {/* group subtotal */}
                    {group.rows?.length > 0 && (
                      <tr className="tbx-subtotal-row">
                        <td style={{ paddingLeft:28 }}>Subtotal — {group.type}</td>
                        <td className="tbx-td-num">
                          {groupDebit > 0 ? fmt(groupDebit) : "—"}
                        </td>
                        <td className="tbx-td-num">
                          {groupCredit > 0 ? fmt(groupCredit) : "—"}
                        </td>
                      </tr>
                    )}
                  </React.Fragment>
              ...
              {/* grand total */}
              <tr className="tbx-grand-row">
                <td>Grand Total &nbsp;·&nbsp; {totalRows} accounts</td>
                <td className="tbx-td-grand">{fmtR(data.totalDebit)}</td>
                <td className="tbx-td-grand">{fmtR(data.totalCredit)}</td>
              </tr>
            </tbody>
          </table>
        ...
        <p style={{ textAlign:"center", color:"#94a3b8", fontSize:12, marginTop:14,
          ...
          {totalRows} account{totalRows !== 1 ? "s" : ""} across {data.groups?.length || 0} categories
        </p>
