  ...
  return (
    <div className="ce-section">
      <div className="ce-section-head">
        ...
        {badge != null && (
          <span className="ce-badge" style={{ marginLeft:"auto" }}>{badge}</span>
        )}
      </div>
      ...
      <div className="ce-wrap">
        ...
        <div style={{ marginBottom:24 }}>
          <p style={{ fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:".1em",
            color:"#9ca3af", marginBottom:4 }}>HR Management</p>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:0 }}>
            ...
            <div className="ce-grid-2" style={{ marginBottom:16 }}>
              <Field label="First Name">
                <input name="firstName" value={formData.firstName} onChange={handleChange}
                  placeholder="e.g. Muhammad" className="ce-input" required />
              </Field>
              <Field label="Last Name">
                <input name="lastName" value={formData.lastName} onChange={handleChange}
                  placeholder="e.g. Ali" className="ce-input" required />
              </Field>
            </div>
            <div className="ce-grid-2" style={{ marginBottom:16 }}>
              <Field label="CNIC" error={errors.cnic}>
                <input name="cnic" value={formData.cnic} onChange={handleChange}
                  ...
                  className={`ce-input mono${errors.cnic ? " ce-input-err" : ""}`}
                  required />
              </Field>
              <Field label="Mobile" error={errors.mobile}>
                <input name="mobile" value={formData.mobile} onChange={handleChange}
                  className={`ce-input mono${errors.mobile ? " ce-input-err" : ""}`}
                  required />
              </Field>
            </div>
            <div className="ce-grid-2">
              <Field label="Email Address">
                <input name="email" type="email" value={formData.email} onChange={handleChange}
                  placeholder="name@example.com" className="ce-input" required />
              </Field>
              <Field label="Home Address">
                <input name="address" value={formData.address} onChange={handleChange}
                  placeholder="Street, City" className="ce-input" />
              </Field>
            </div>
          </Section>
          ...
          >
            {errors.role && (
              <p style={{ fontSize:11.5, color:"#ef4444", marginBottom:10, fontWeight:500 }}>{errors.role}</p>
            )}
            <div className="ce-role-grid">
              {roles.map(r => (
                <div
                  key={r.value}
                  className={`ce-role-card${formData.role === r.value ? " selected" : ""}`}
            ...
            <div className="ce-grid-2">
              <Field label="Username">
                <input name="username" value={formData.username} onChange={handleChange}
                  placeholder="e.g. m.ali" className="ce-input mono" required />
              </Field>
                    ...
                    <div style={{ display:"flex", gap:4, marginTop:7 }}>
                      {[1,2,3,4].map(i => (
                        <div key={i} style={{
                          ...
                          background: i <= strength.score ? strength.color : "#f3f4f6",
                          transition:"background .3s",
                        }}/>
                      ))}
                    </div>
                    <p style={{ fontSize:11, marginTop:4, fontWeight:600, color: strength.color }}>
                      {strength.label}
                    </p>
                  </div>
                )}
          ...
          <Section
            ...
            badge={formData.allowedRoutes.length}
          >
            <div className="ce-route-actions">
              <button type="button" className="ce-route-action-btn" onClick={selectAllRoutes}>
                Select All
              </button>
              <button type="button" className="ce-route-action-btn" onClick={clearAllRoutes}>
                Clear All
              </button>
            ...
            <div className="ce-route-grid">
              {routesList.map(route => {
                ...
                return (
                  <div
                    key={route}
                    className={`ce-route-pill${active ? " active" : ""}`}
          ...
          >
            <div className="ce-dropzone">
              ...
              <svg width={32} height={32} fill="none" viewBox="0 0 24 24" stroke="#c7d2fe" strokeWidth={1.5}
