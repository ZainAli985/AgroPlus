// WeightBridge.jsx — vehicles loaded from /api/profile/vehicles (per-mill custom)
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-baseline justify-between">
        <label className="block text-[11px] font-bold text-zinc-400 uppercase tracking-widest">{label}</label>
...
export default function WeightBridge() {
  ...
  const [vehicles, setVehicles] = useState([]);  // loaded from API
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (pData.success || Array.isArray(pData.products)) setProducts(pData.products || pData);
  ...
  return (
    <SidebarLayout>
      <style>{`
        ...
        @keyframes wbPop   { from{opacity:0;transform:scale(.9)} to{opacity:1;transform:scale(1)} }
        ...
        .wbt-track { width:46px;height:26px;border-radius:999px;background:#e4e4e7;transition:background .2s;position:relative;flex-shrink:0; }
        .wbt-thumb { position:absolute;top:3px;left:3px;width:20px;height:20px;border-radius:50%;background:white;box-shadow:0 1px 4px rgba(0,0,0,.2);transition:left .2s; }
        ...
        <div className="max-w-2xl mx-auto mb-6">
          <div className="bg-zinc-900 rounded-2xl shadow-2xl">
            <div className="bg-amber-400 px-8 py-1.5 flex items-center justify-between rounded-t-2xl">
              <span className="wb-title text-zinc-900 text-xs font-bold uppercase tracking-[.18em]">Operations Module</span>
              <span className="wb-mono text-zinc-900 text-xs">{nowStr}</span>
            </div>
            <div className="px-8 pt-7 pb-7 flex items-center justify-between">
              <div>
                <h1 className="wb-title text-5xl font-extrabold text-white tracking-tight" style={{lineHeight:"1.15"}}>WEIGHT BRIDGE</h1>
                <p className="text-zinc-400 text-sm mt-1.5 font-light tracking-wide">Vehicle weight recording system</p>
              </div>
              <div className="flex gap-1.5 bg-zinc-800 p-1.5 rounded-2xl">
                {[{ id:"new", label:"New Entry", num:"①" }, { id:"second", label:"2nd Weight", num:"②" }].map(tab => (
                  <button key={tab.id} onClick={() => { setView(tab.id); resetForm(); }}
                    className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all wb-title uppercase tracking-wider ${view===tab.id ? "bg-amber-400 text-zinc-900 shadow" : "text-zinc-400 hover:text-white"}`}
                  >
                    {tab.num} {tab.label}
                  </button>
                ))}
                ...
                <div className="px-6 pt-6 pb-5 border-b border-zinc-100">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Vendor Name">
                      <input type="text" name="vendorName" placeholder="e.g. Ali Traders" value={form.vendorName} onChange={handleChange} className={inp} required/>
                    </Field>
                    <Field label="Vehicle Number" hint="Plate No.">
                      <input type="text" name="vehicleNumber" placeholder="LEA-1234" value={form.vehicleNumber} onChange={handleVehicleNumberChange} className={`${inp} wb-mono plate-input`}/>
                    </Field>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    <Field label="Product">
                      <select name="productId" value={form.productId}
                        onChange={e => { const s = products.find(p => p._id === e.target.value); setForm(p => ({ ...p, productId: e.target.value, productName: s?.productName || "" })); }}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Vehicle Type">
                        {vehiclesLoading ? (
                          <div className={`${inp} bg-zinc-50 text-zinc-400`}>Loading vehicles…</div>
                        ) : vehicles.length === 0 ? (
                          <div>
                            ...
                            <p className="text-[10px] text-amber-600 mt-1 font-medium">
                              ⚠ Add vehicles in Profile → Vehicles tab
                            </p>
                          </div>
                        ) : (
                          <select name="vehicleType" value={form.vehicleType} onChange={handleVehicleChange} className={inp} required>
                  ...
                  <div className="grid grid-cols-2 gap-4 items-end">
                    <Field label="First Weight" hint="kg">
                      <div className="relative">
                        <input type="number" name="firstWeight" placeholder="0" value={form.firstWeight} onChange={handleChange}
                    ...
                    <div>
                      ...
                      <button type="button" onClick={() => setForm(p => ({ ...p, firstWeightWithDriver: !p.firstWeightWithDriver }))}
              ...
              <div className="bg-zinc-900 rounded-2xl overflow-hidden shadow-2xl wb-pop">
                ...
                <div className="grid grid-cols-2 divide-x divide-zinc-800">
                  <div className="px-8 py-5 text-center">
                    ...
                    <p className="wb-mono text-4xl font-bold text-white">{Number(form.firstWeight).toLocaleString()}</p>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-zinc-100 px-6 py-5">
                ...
                <div className="flex gap-3">
                  <div className="flex flex-1 items-center border border-zinc-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-amber-400 focus-within:border-amber-400 transition">
                    <span className="wb-mono text-sm font-bold text-zinc-400 bg-zinc-50 px-4 py-3.5 border-r border-zinc-200 whitespace-nowrap select-none">WB-</span>
                    <input type="text" value={form.invoiceCode.replace("WB-", "")}
                ...
                <div className="space-y-4">
                  <div className="bg-zinc-900 rounded-2xl px-6 py-5 grid grid-cols-3 gap-4 items-center">
                    ...
                    <div className="text-center border-x border-zinc-800 px-4">
                      <p className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">First Weight</p>
                      <p className="wb-mono text-3xl font-bold text-white">{Number(form.firstWeight || 0).toLocaleString()}</p>
                      <p className="text-zinc-600 text-xs">kg</p>
                    </div>
                    <div className="text-right">
                      {form.completed
                        ? <span className="inline-flex items-center gap-1.5 bg-emerald-500 text-white text-xs font-bold px-3 py-1.5 rounded-full wb-title uppercase tracking-wider">✓ Done</span>
                        : <span className="inline-flex items-center gap-1.5 bg-amber-400 text-zinc-900 text-xs font-bold px-3 py-1.5 rounded-full wb-title uppercase tracking-wider">Pending</span>
                      }
                    </div>
                  </div>
                      ...
                      <div className="px-6 pt-6 pb-5">
                        ...
                        <div className="grid grid-cols-2 gap-4 items-end">
                          <Field label="Second Weight" hint="kg">
                            <div className="relative">
                              <input type="number" name="secondWeight" placeholder="0" value={form.secondWeight} onChange={handleChange}
                                className={`${inp} wb-mono text-2xl font-bold pr-14 py-4`} required/>
                              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-400 pointer-events-none">KG</span>
                            </div>
                          </Field>
                          <div>
                            <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Driver</p>
                            <button type="button" onClick={() => setForm(p => ({ ...p, secondWeightWithDriver: !p.secondWeightWithDriver }))}
                  ...
                  {hasNetWeight && (
                    <div className="bg-zinc-900 rounded-2xl overflow-hidden shadow-2xl wb-pop">
                      <div className="bg-emerald-500 px-6 py-2.5 flex items-center justify-between">
                        <span className="wb-title text-white text-xs font-bold uppercase tracking-[.15em]">✓ Net Weight Result</span>
                      ...
                      <div className="px-8 py-8 text-center border-b border-zinc-800">
                        ...
                        <p className="wb-mono font-bold text-white leading-none" style={{fontSize:"4.5rem"}}>{Number(form.netWeight).toLocaleString()}</p>
